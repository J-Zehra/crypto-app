...
export const NewestCoins = () => {

    const { data: coins } = useGetGlobalStatsQuery();
    const { data: newCoin1 } = useGetCryptoQuery({ id: coins?.data?.newestCoins[0]?.uuid })
    const { data: newCoin2 } = useGetCryptoQuery({ id: coins?.data?.newestCoins[1]?.uuid })
    const { data: newCoin3 } = useGetCryptoQuery({ id: coins?.data?.newestCoins[2]?.uuid })
